...
export class MissionskillComponent implements OnInit {
  ...
  openDeleteSkillModal(id: number) {
    this.skillId = id; // Store the ID for deletion
    this.deleteSkillmodal.show(); // Show the modal
  }
  ...
  DeleteSkillModal() {
    this.service.DeleteMissionSkill(this.skillId).subscribe((data: any) => {
      if (data.result == 1) {
        ...
        setTimeout(() => {
          this.GetMissionSkillList(); // Refresh the list
        }, 1000);
      } else {
        this.toast.error({ detail: "ERROR", summary: data.message, duration: 3000 });
      }
  ...
  editMissionSkill(id: number) {
    this.route.navigate(['admin/updateMissionSkill', id]);
  }
}
