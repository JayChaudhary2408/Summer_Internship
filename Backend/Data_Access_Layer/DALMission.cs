       ...
       public List<Missions> MissionList()
{
    return _cIDbContext.Missions
        .Where(m => !m.IsDeleted) // This filters out deleted missions
        .ToList();
}
     ...
     public string UpdateMission(Missions mission)
{
    ...
    try
    {
        ...
        if (!missionExists)
        {
            ...
            {
                // Update only the necessary fields, excluding ModifiedDate
                ...
                // Do not set missionToUpdate.ModifiedDate
       ...
       public List<MissionApplication> MissionApplicationList()
{
    ...
    try
    {
        missionApplicationList = _cIDbContext.MissionApplication
            .Where(ma => !ma.IsDeleted && !ma.Status) // Added constraint for Status
        ...
        public string MissionApplicationApprove(int id)
{
    try
    {
        // Attempt to find the mission application
        var missionApplication = _cIDbContext.MissionApplication.FirstOrDefault(m => m.Id == id);
        
        // Check if the mission application was found
        if (missionApplication != null)
        {
        ...
        {
            // Log or throw an exception if not found
            return $"Mission application with ID {id} not found.";
        }
    }
    catch (Exception ex)
    {
        ...
        Console.WriteLine($"An error occurred: {ex.Message}");
        return "An error occurred while approving the mission.";
    }
}
